...
public class DepAnnounceManager extends CommonManager {

	private DepAnnounceService service;
	
	public DepAnnounceManager(Logger logger) {
		super(logger);
		this.service = new DepAnnounceService(logger);
	}
	...
	public void initSearchAnnounce(AnnounceModel modelResponse, LoginUser loginUser) throws Exception {
		...
		try {
			...
			modelResponse.setLstDepAnnounceType(selectItemManager.getGlobalSelectitem(GlobalType.DEPARTMENT_ANNOUNCE, loginUser.getLocale()));
			// Combo Status
	...
	public void initAddEditAnnounce(AnnounceModel modelResponse, AnnounceModel modelReq, boolean isEdit, LoginUser loginUser) throws Exception {
		...
		try {
			...
			if(isEdit) {
				...
				id = service.validateIdDep(modelReq.getAnnounce(), loginUser);
				// search by id
				modelResponse.setAnnounce(service.searchByIdDep(conn, id, loginUser));
			}
			
			// Combo AnnounceType
			SelectItemManager selectItemManager = new SelectItemManager(getLogger());
			modelResponse.setLstAnnounceType(selectItemManager.getGlobalSelectitem(GlobalType.DEPARTMENT_ANNOUNCE, loginUser.getLocale()));
			// Combo Status
			modelResponse.setLstStatus(selectItemManager.getGlobalSelectitem(GlobalType.ACTIVE_STATUS, loginUser.getLocale()));
		} finally {
			CCTConnectionUtil.close(conn);
	...
	public void searchAnnounce(AnnounceModel modelReq, CommonSearchResult<AnnounceSearch> searchResult,LoginUser loginUser) throws Exception {
		...
		try {
			// Validate
			service.validateSearchDep(modelReq.getCriteria(), loginUser);
			
			conn = new CCTConnectionProvider().getConnection(conn, DBLookup.E_EXT_API.getLookup());
			// Search
			searchResult.setListResult(service.searchDepAnnounce(conn, modelReq.getCriteria(), loginUser));
		} finally {
			CCTConnectionUtil.close(conn);
		}
	}
	...
	public void addAnnounce(AnnounceModel modelReq, 
		...
		try {
			...
			//1. Validate
			service.validateAddEditDep(modelReq.getAnnounce(), file, loginUser, false);
			//2. Manage file
			...
			//3. Insert
			service.addAnnounceDep(conn, modelReq.getAnnounce(), pathCover, pathFile, category);
			// Upload
			service.uploadFileDep(fileCover, pathCover, file, pathFile, sharedPath);
	...
	public void editAnnounce(AnnounceModel modelReq, 
		...
		try {
			String sharedPath = ParameterConfig.getApplication().getSharePath();
			//1. Validate
			String id = service.validateAddEditDep(modelReq.getAnnounce(), file, loginUser, true);
			...
			//3. Update
			service.updateAnnounceDep(conn, modelReq.getAnnounce(), pathCover, pathFile, id);
			// Upload
			service.uploadFileDep(fileCover, pathCover, file, pathFile, sharedPath);
	...
	public void updateActiveAnnounce(AnnounceModel modelReq,LoginUser loginUser) throws Exception {
		...
		try {
			// Validate
			String ids = service.validateActiveDep(modelReq.getAnnounce(), loginUser);
			
			conn = new CCTConnectionProvider().getConnection(conn, DBLookup.E_EXT_API.getLookup());
			// Update status
			service.updateActiveAnnounceDep(conn, modelReq.getAnnounce(), ids);
		} finally {
			CCTConnectionUtil.close(conn);
		}
	}
